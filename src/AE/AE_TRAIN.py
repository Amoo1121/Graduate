...
def train(cfg):
    ...
    if os.path.exists('./NEW_Network_Model'):
        model_dict = torch.load('./NEW_Network_Model/network_model.pth')
        model.load_state_dict(model_dict)  # 每次跟随上次进度训练
    if cfg.use_gpu:
        model.cuda()  # 是否使用cuda
    ...
    for epoch in range(cfg.max_epoch):
        ...
        if epoch % 30 == 0:
            ...
            torch.save(model_dict, './New_Network_Model/network_model.pth')  # 每30次保存一次网络模型
